    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._cur_best_program_score = (
            float("-inf")
            if self._num_objs < 2
            else [float("-inf") for _ in range(self._num_objs)]
        )
            ...
            if score is not None:
                for i in range(self._num_objs):
                    if score[i] > self._cur_best_program_score[i]:
                        ...
                        self._cur_best_program_sample_order[i] = (
                            self.__class__._num_samples
                        )
                        self._write_json(function, record_type="best")
        ...
        if not resume_mode:
            ...
            if self._log_style == "complex":
                ...
                if score is None:
                    if self._num_objs < 2:
                        print(
                            f"Sample{self.__class__._num_samples}: Score=None    Cur_Best_Score={self._cur_best_program_score: .3f}"
                        )
                    else:
                        print(
                    ...
                    if self._num_objs < 2:
                        print(
                            f"Sample{self.__class__._num_samples}: Score={score: .3f}     Cur_Best_Score={self._cur_best_program_score: .3f}"
                        )
                    else:  # TODO: MEoH: only support 2 objs
                        print(
                            f"Sample{self.__class__._num_samples}: Score=[{score[0]: .3f}, {score[1]: .3f}]     Cur_Best_Score=[{self._cur_best_program_score[0]: .3f}, {self._cur_best_program_score[1]: .3f}]"
                        )
