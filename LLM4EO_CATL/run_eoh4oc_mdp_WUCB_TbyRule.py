...
from Agent.op_select_agent import (
    ...
    UCBDecision,
    Window_UCBDecision,  # according the UCB to decise updating template
    LLMDecisionPolicy,  # operator and template decided by llm
    TemplateRewriteDecision,  # according the UCB to decise updating template
...
if __name__ == "__main__":
    ...
    timestamp = "WUCB_TbyRule"  # debug 限定不变
    ...
    for operator_name in operators_to_optimize:
        cached = load_warmup_batch("FJSP_OC_TMAX_WUCB_EPNT", operator_name)
        # 记录template
        ...
        eoh.run()
        # 获取pops
        pop_obj = getattr(eoh, "_population", None)
        hist = getattr(pop_obj, "_history_sampling_pops", None) or getattr(
            pop_obj, "history_sampling_pops", None
        )
        pop = hist() if callable(hist) else (hist or pop_obj)

        if not pop:
            logger.log_warning(f"[Warmup] No population for {operator_name}, skip")
            continue
        # save pops
        individuals = _simple_population(pop)
    ...
    for cycle in range(max_cycles):
        ...
        # continue
        logger.log_info(
            f"[Decision] pick operator={operator_name}, n_samples={generation_num}"
        )
        ...
        ope_pops = hist() if callable(hist) else (hist or pop_obj)

        if not ope_pops:
            logger.log_warning(f"[Warmup] No population for {operator_name}, skip")
            continue
        else:
            es = storages.record_from_eoh(
        ...
        # 7) 从该批挑最优个体作为候选
        best_function = max(
            ope_pops, key=lambda ind: getattr(ind, "score", float("-inf"))
        )
        ...
        best_after = new_score if accepted else best_score

        # === 新增：记录“每次决策” ===
        logger.log_decision(
            cycle=cycle,
            op_features=op_features,  # storages.build_features()
            operator_name=operator_name,
            do_rewrite=do_rewrite,
            template=template,  # 本轮使用的模板文本（可能是默认或重写后的）
            accepted=accepted,  # 是否更新算子
            current_ops=current_operators,  # 更新前的组合（源码不会直接写日志，只记长度+sha1）
            candidate_ops=candidate_operators,  # 本轮候选组合
            new_score=new_score,
            best_score_before=best_before,
            best_score_after=best_after,
            extra=extra_metrics,  # 可选
        )
        # 9) 贪婪接受
