...
class MCTS:
    ...
    def _ini_state(self, node, origin_state):
        ...
        while len(node.state) < 4:
            operator_name = self.operators_to_optimize[len(node.state)]
        ...
        half_n = self.max_sampling_num // 2  # 前一半迭代的代次数
        # 计算采样间隔
        if half_n > start_gen:
    ...
    def expand(self, generation, node):
        ...
        next_var_index = random.randint(0, len(self.operators_to_optimize) - 1)
...
if __name__ == "__main__":
    ...
    timestamp = "MCT_FIX_4Tuple"  # fixed timestamp for result comparison
    ...
    main_args = {
        "n_evals": 1,  # 每次评估的重复次数 catl为6不需要调整
        "obj_list": ["makespan", "total_load"],
        "generation_num": 1,
        "pop_size": 50,  # 种群大小
        "eoh_max_generations": 15,
        "eoh_max_sample_nums": 15,  # max_sample_nums 大模型生成算子的个数 达到这个个数或者是迭代次数都会停止
    ...
    # 创建MCTS对象
    mcts = MCTS(main_args, llm, max_iterations=30, max_simulations=1)
    # 进行MCTS搜索并保存每一代的信息
    history = mcts.search()
