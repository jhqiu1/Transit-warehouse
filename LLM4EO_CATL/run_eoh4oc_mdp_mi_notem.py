...
from _storage_operators import Storages  # 极简 storages
from Agent.op_select_agent import RandomEqualAllocator, UCBDecision, LLMDecisionPolicy
...
if __name__ == "__main__":
    ...
    eoh_max_sample_nums = 50  # 最小评估次数
    eoh_pop_size = 5  # 最小种群
    eoh_num_samplers = 1  # 单线程
    eoh_num_evaluators = 1  # 单线程
    ...
    timestamp = "RAMDOM2"  # debug 限定不变
    exp_name = "FJSP_OC_LLM_Eva"
    exp_dir = "FJSP_OC_MDP_" + timestamp
    ...
    for operator_name in operators_to_optimize:
        cached = load_warmup_batch("FJSP_OC_MDP_RANDOM", operator_name)
        # 记录template
        ...
        res = storages.record_batch(
            operator=operator_name, individuals=individuals, cycle=0, step=None
        )
        logger.log_info(
    ...
    for cycle in range(max_cycles):
        ...
        # 2) 决策：算子 + 本轮采样数
        operator_name, n_samples = policy.decide(cycle, op_features)

        print(f"[Decision] pick operator={operator_name}, n_samples={n_samples}")

        # continue
        logger.log_info(
            f"[Decision] pick operator={operator_name}, n_samples={n_samples}"
        )

        # 3) 准备模板（此处用固定模板；如需 LLM，请替换为 prompt 调用）
        # template = template_dict[operator_name + "_template"]
