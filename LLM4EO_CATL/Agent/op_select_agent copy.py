...
def build_decision_prompt(
    ...
    for op in operators:
        ...
        lines.append(
            f"- operator: {op}\n"
            f"  Latest batch: n={l_n}, mean={_fmt(l_mean)}, best={_fmt(l_best)}, worst={_fmt(l_worst)}, "
...
def validate_llm_decision(d: dict, allowed_ops: List[str]) -> Tuple[bool, str]:
    ...
    if "n_samples" not in d:
        return False, "Missing field 'n_samples'."
    try:
        n = int(d["n_samples"])
        if n < 1:
            return False, "'n_samples' must be integer >=1."
    except Exception:
        return False, "'n_samples' must be integer."
    return True, ""
