...
class MCTS:
    ...
    def expand(self, generation, node):
        ...
        if generation in self.target_generations:
            self.samppling_vars(node)  # sampling new template
        if len(node.state) >= 4 and generation != 0:  # 完整状态（4个变量）时停止扩展
            return
        # 为每个未赋值的变量从分布中采样生成新的子节点
    ...
    def simulate(self, node):
        ...
        while len(current_state) < 4:  # 如果状态不完整，补充缺失的变量
            next_var_index = len(current_state)
            next_value = random.choice(
                self.history_var[next_var_index]
            )  # 从分布中直接采样
    ...
    def plot_convergence(self):
        ...
        if not os.path.exists("outputs"):
            os.makedirs("outputs")
        plt.savefig("global_utils/outputs/convergence_curve_sampling.jpg")
        plt.close()
...
# 保存到txt文件
mcts.save_to_txt("global_utils/outputs/mcts_history_sampling.txt")
