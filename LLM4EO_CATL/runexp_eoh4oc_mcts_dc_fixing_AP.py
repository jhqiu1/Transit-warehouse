...
if __name__ == "__main__":
    ...
    # 定义不同的Number of initial added prompt
    ap_list = [1, 3, 5, 7]
    ...
    for ap_value in ap_list:
        ...
        timestamp = "MCT_FIX_fixing_AP_" + str(ap_value)  # fixed timestamp for result comparison
        exp_name = "FJSP_OC_TMAX"
        exp_dir = "FJSP_OC_" + timestamp
        log_dir = "outputs_AP/" + exp_dir + "/eoh_log/{}"
        ...
        # 创建MCTS对象
        mcts = MCTS(main_args, llm, storages, max_iterations=30, max_simulations=1,max_sampling_num=ap_value)
        # 进行MCTS搜索并保存每一代的信息
        history = mcts.search()
