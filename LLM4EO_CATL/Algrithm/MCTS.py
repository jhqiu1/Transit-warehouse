...
def black_box_objective_function(variables):
    ...
    return sum(variables) + random.uniform(-0.5, 0.5)  # 添加一些噪声模拟黑盒不确定性
...
class MCTS:
    def __init__(self, max_iterations=1000, max_simulations=10, max_sampling_num=4):
    ...
    def _ini_state(self, node, origin_state):
        for next_var_index in range(len(origin_state)):
            ...
            while len(self.history_var[next_var_index]) < self.max_sampling_num:
                self.history_var[next_var_index].append(
                    self.sampling_value(next_var_index)
                )
    ...
    def expand(self, node):
        ...
        for value in self.history_var[next_var_index]:
    ...
    def simulate(self, node):
        ...
        while len(current_state) < 4:
            next_var_index = len(current_state)
    ...
    def plot_convergence(self):
        ...
        if not os.path.exists("outputs"):
            os.makedirs("outputs")
        plt.savefig("outputs/convergence_curve.jpg")
        plt.close()


# 创建MCTS对象
mcts = MCTS(max_iterations=100, max_simulations=3)
...
# 保存到txt文件
mcts.save_to_txt("outputs/mcts_history.txt")
