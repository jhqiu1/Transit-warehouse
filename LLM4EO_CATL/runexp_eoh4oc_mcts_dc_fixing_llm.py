...
if __name__ == "__main__":
    ...
    for model in llm_list:
        ...
        ##############################
        # initialize LLM
        llm._model = model  # 定义不同模型
        if model == "chatgpt-4o-latest":
            llm._host = "az.gptplus5.com"
            llm._key = "sk-9jf8H6xVpA3wRLvk5b55Ff687655482a8cB86aF88b8d4f34"
        # print(llm.draw_sample("你是什么模型，什么版本，什么供应厂商的"))
        # continue

        # initialize templates

        timestamp = "MCT_FIX_fixing_" + model  # fixed timestamp for result comparison
        exp_name = "FJSP_OC_TMAX"
        exp_dir = "FJSP_OC_" + timestamp
        log_dir = "outputs_llm/" + exp_dir + "/eoh_log/{}"

        max_cycles = 3  # CD 迭代次数
        main_args = {
            "n_evals": 3,  # 每次评估的重复次数 catl为6不需要调整
            "obj_list": ["makespan", "total_load"],
            "generation_num": 10,
            "pop_size": 100,  # 种群大小
            "eoh_max_generations": 3,
        ...
        for operator_name in operators_to_optimize:
            ...
            cached = load_warmup_batch(exp_dir, operator_name)
            # 记录template
