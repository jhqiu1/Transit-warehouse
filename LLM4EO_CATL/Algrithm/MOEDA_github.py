...
def reference_points(npop, dim):
    ...
    while combination(h1 + dim, dim - 1) <= npop:
        h1 += 1
    points = (
        np.array(list(combinations(np.arange(1, h1 + dim), dim - 1)))
        - np.arange(dim - 1)
        - 1
    )
    points = (
        np.concatenate((points, np.zeros((points.shape[0], 1)) + h1), axis=1)
        - np.concatenate((np.zeros((points.shape[0], 1)), points), axis=1)
    ) / h1
    if h1 < dim:
        h2 = 0
        while (
            combination(h1 + dim - 1, dim - 1) + combination(h2 + dim, dim - 1) <= npop
        ):
            h2 += 1
        if h2 > 0:
            temp_points = (
                np.array(list(combinations(np.arange(1, h2 + dim), dim - 1)))
                - np.arange(dim - 1)
                - 1
            )
            temp_points = (
                np.concatenate(
                    (temp_points, np.zeros((temp_points.shape[0], 1)) + h2), axis=1
                )
                - np.concatenate(
                    (np.zeros((temp_points.shape[0], 1)), temp_points), axis=1
                )
            ) / h2
...
def crossover(parent1, parent2, lb, ub, dim, pc, eta_c):
    ...
    if np.random.random() < pc:
        ...
        offspring1 = np.where(
            ((offspring1 >= lb) & (offspring1 <= ub)),
            offspring1,
            np.random.uniform(lb, ub),
        )
        offspring2 = np.where(
            ((offspring2 >= lb) & (offspring2 <= ub)),
            offspring2,
            np.random.uniform(lb, ub),
        )
...
def mutation(individual, lb, ub, dim, pm, eta_m):
    ...
    if np.random.random() < pm:
        ...
        individual[temp] += (ub[temp] - lb[temp]) * (
            (2 * mu[temp] + (1 - 2 * mu[temp]) * (1 - delta1[temp]) ** (eta_m + 1))
            ** (1 / (eta_m + 1))
            - 1
        )
        temp = np.logical_and(site, mu > 0.5)
        individual[temp] += (ub[temp] - lb[temp]) * (
            ...
            - (
                2 * (1 - mu[temp])
                + 2 * (mu[temp] - 0.5) * (1 - delta2[temp]) ** (eta_m + 1)
            )
            ** (1 / (eta_m + 1))
        )
        individual = np.where(
            ((individual >= lb) & (individual <= ub)),
            individual,
            np.random.uniform(lb, ub),
        )
    return individual
...
def main(npop, iter, lb, ub, T=20, pc=1, pm=1, eta_c=20, eta_m=20):
    ...
    sigma = squareform(
        pdist(V, metric="euclidean"), force="no", checks=True
    )  # distances between weight vectors
    ...
    for t in range(iter):
        ...
        for i in range(npop):
            ...
            for j in B[i]:
                if np.max(V[j] * np.abs(off_obj - z)) < np.max(
                    V[j] * np.abs(objs[j] - z)
                ):
                    pop[j] = off
                    objs[j] = off_obj
